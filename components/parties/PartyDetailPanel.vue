<template>
  <div class="detail" :class="{ 'detail--empty': !party }">
    <div v-if="!party" class="empty">
      ...
      <p class="empty-title">{{ t('Select a party') }}</p>
      <p class="empty-sub">
        {{
          t('Pick someone on the left to see their activity, balances, and recent transactions.')
        }}
      </p>
    </div>
      ...
      <div :key="party.id" class="detail-body">
        <header class="hero surface" :class="netTone">
          ...
          >
            <defs>
              ...
              <radialGradient id="hero-bloom" cx="100%" cy="50%" r="60%">
            ...
            <rect x="0" y="0" width="800" height="80" fill="url(#hero-dots)" />
            <circle cx="780" cy="40" r="120" fill="url(#hero-bloom)" />
          </svg>
          ...
          <div class="hero-meta">
            <h2 class="hero-name">{{ party.name }}</h2>
            <span class="hero-sub">
              <span class="hero-type">{{ displayType }}</span>
              <template v-if="party.description">
                <span class="hero-dot">·</span>
                <span class="hero-desc">{{ party.description }}</span>
              </template>
            </span>
          </div>

          <div class="hero-actions">
            <button class="hero-action" :aria-label="t('Edit')" @click="$emit('edit', party)">
              <Edit3 :size="14" />
            </button>
            <button
        ...
        <section class="totals">
          <div class="total total--income">
            <span class="total-label">{{ t('Received') }}</span>
            <p class="total-value">{{ formatter(party.receivedAmount || 0, currency) }}</p>
          </div>
          <div class="total-divider" />
          <div class="total total--expense">
            <span class="total-label">{{ t('Spent') }}</span>
            <p class="total-value">{{ formatter(party.spentAmount || 0, currency) }}</p>
          </div>
          <div class="total-divider" />
          <div class="total total--net">
            <span class="total-label">{{ t('Net') }}</span>
            <p class="total-value" :class="netValue >= 0 ? 'tone-income' : 'tone-expense'">
              {{ formatter(netValue, currency) }}
            </p>
          </div>
        </section>
        ...
        <section class="chart-card">
          <header class="chart-head">
            ...
            <span class="chart-legend">
              <span class="legend-dot legend-dot--income" />{{ t('Received') }}
              <span class="legend-dot legend-dot--expense" />{{ t('Spent') }}
            </span>
          </header>
          <svg
            v-if="hasActivity"
          ...
          >
            ...
            />
            <polyline
              :points="linePoints('receivedAmount')"
              fill="none"
            ...
            <g v-for="(m, i) in monthly" :key="`pt-${i}`">
              <circle
                v-if="m.receivedAmount > 0"
                :cx="xAt(i)"
                :cy="yAt(m.receivedAmount)"
                r="3"
                fill="var(--color-income)"
                vector-effect="non-scaling-stroke"
              />
              <circle
                v-if="m.spentAmount > 0"
                :cx="xAt(i)"
                :cy="yAt(m.spentAmount)"
                r="3"
        ...
        <section class="recent">
          <header class="recent-head">
            ...
            <NuxtLink :to="`/transactions?partyId=${party.id}`" class="recent-link">
              {{ t('View all') }}
          ...
          <ul v-if="recentTransactions.length" class="recent-list">
            <li v-for="tx in recentTransactions" :key="tx.id" class="recent-row">
              <div class="recent-row-left">
                ...
                <div class="recent-text">
                  <span class="recent-name">{{
                    tx.category || tx.description || t('Transaction')
                  }}</span>
              ...
              <span
                ...
                "
              >
                {{ formatter(parseAmount(tx.amount).value, currency) }}
              </span>
            </li>
          </ul>
          <div v-else class="recent-empty">
            {{ t('No transactions yet with this party.') }}
          </div>
        </section>
      </div>
    </Transition>
...
const displayType = computed(() => {
  ...
  return map[type] || (props.party?.type ? props.party.type : t('Party'));
});

const netValue = computed(() => {
  if (!props.party) return 0;
  return Number(props.party.receivedAmount || 0) - Number(props.party.spentAmount || 0);
});

const netTone = computed(() => {
  if (!props.party) return 'surface--brand';
...
const resolvedIcon = computed(() => {
  const icon = props.party?.icon;
  let iconValue = '';
  if (icon) {
    if (typeof icon === 'string') iconValue = icon;
    else if (icon.path) iconValue = icon.path;
    else if (icon.content) iconValue = icon.content;
  }
  if (!iconValue) {
    return props.party?.type === 'individual' ? LucideIcons.User : LucideIcons.Building2;
  }
  return (
    LucideIcons[iconValue] ||
    (props.party?.type === 'individual' ? LucideIcons.User : LucideIcons.Building2)
  );
});
...
const partyTransactions = computed(() => {
  ...
  return props.transactions
    .filter((tx) => tx.partyId === props.party.id)
    .sort(
...
const monthly = computed(() => {
  ...
  partyTransactions.value.forEach((tx) => {
    ...
    if (tx.type === 'INCOME' || tx.type === 'income') bucket.receivedAmount += v;
    else bucket.spentAmount += v;
  });
  return out;
});
...
const maxMonthValue = computed(() => {
  ...
  monthly.value.forEach((m) => {
    max = Math.max(max, m.receivedAmount, m.spentAmount);
  });
  return max || 1;
});
